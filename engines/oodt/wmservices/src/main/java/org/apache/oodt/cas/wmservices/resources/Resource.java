...
public abstract class Resource {
  ...
  public File getContextPkgReposDir() throws Exception {
    Object repositoryDirObject = context.getAttribute(
      WmServicesServlet.ATTR_NAME_PKG_REPO_DIR
    );
    if (repositoryDirObject != null && repositoryDirObject instanceof File) {
      return (File) repositoryDirObject;
    }
    String message =
      "Unable to retrieve packaged repository directory from the servlet context.";
  ...
  public XmlRpcWorkflowManagerClient getContextClient() throws Exception {
    ...
    Object clientObject = context.getAttribute(
      WmServicesServlet.ATTR_NAME_CLIENT
    );
    if (
      clientObject != null &&
    ...
    String message =
      "Unable to retrieve workflow manager client from the " +
      "servlet context.";
    LOGGER.log(Level.WARNING, message);
    throw new Exception(message);
  }
