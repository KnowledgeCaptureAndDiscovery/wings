 ...
 * arg3 = arg1 ^^ arg2
 */
public class Log extends BaseBuiltin {
	...
	public boolean bodyCall(Node[] args, int length, RuleContext context) {
		...
		if (n1.isLiteral() && n2.isLiteral()) {
			...
			if (v1 instanceof Number && v2 instanceof Number) {
				...
				if (v1 instanceof Float || v1 instanceof Double || v2 instanceof Float
						|| v2 instanceof Double) {
					sum = Util.makeDoubleNode(Math.log(nv1.doubleValue())/Math.log(nv2.doubleValue()));
				} else {
					sum = Util.makeDoubleNode(Math.log(nv1.longValue())/Math.log(nv2.longValue()));
				}
