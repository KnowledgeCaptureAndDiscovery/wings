...
public class ComponentReasoningKB
  extends ComponentKB
  implements ComponentReasoningAPI {
  ...
  public ComponentReasoningKB(Properties props) {
    ...
    this.useRules =
      Boolean.parseBoolean(props.getProperty("use_rules", "true"));
  ...
  private void initializeMetrics() {
    ...
    this.metricTriples.addAll(
        this.kb.genericTripleQuery(null, rdfsProp, dcPropD)
      );

    // Get a list of all metrics and datametrics properties in the catalog
    this.metricProps =
      this.kb.getSubPropertiesOf(this.objPropMap.get("hasMetrics"), false);
    this.metricProps.addAll(
        this.kb.getSubPropertiesOf(
            this.dataPropMap.get("hasDataMetrics"),
            false
          )
      );
  ...
  protected KBObject copyObjectIntoKB(
    String id,
    String originid,
    KBAPI tkb,
    String includeNS,
    ...
    try {
      ...
      for (KBTriple triple : this.getTriplesForObject(
          id,
          obj,
          includeNS,
          excludeNS,
  ...
  protected KBObject copyObjectClassesIntoKB(
    String id,
    String originid,
    KBAPI tkb,
    String includeNS,
    ...
    try {
      ...
      KBObject obj = this.kb.getIndividual(originid);
      // Add component to the temporary KB (add all its classes explicitly)
      for (KBTriple triple : this.getObjectClassTriples(
          id,
          obj,
          includeNS,
          excludeNS,
  ...
  protected ArrayList<String> getConcreteComponentsForAbstract(String id) {
    if (abscache.containsKey(id)) return abscache.get(id);
    ...
    for (KBObject inst : insts) ids.add(inst.getID());
    this.abscache.put(id, ids);
    return ids;
  }

  protected ArrayList<KBTriple> getTriplesForObject(
    String id,
    KBObject obj,
    String includeNS,
    String excludeNS,
    ...
    if (obj != null) {
      ...
      triples.addAll(
        getObjectClassTriples(id, obj, includeNS, excludeNS, direct)
      );
    }
    return triples;
  }

  protected ArrayList<KBTriple> getObjectClassTriples(
    String id,
    KBObject obj,
    String includeNS,
    String excludeNS,
    ...
    for (KBObject objcls : objclses) {
      if (
        (includeNS != null && objcls.getNamespace().equals(includeNS)) ||
  ...
  private ArrayList<KBObject> getAllClassesOfInstance(KBAPI tkb, String id) {
    ...
    for (KBTriple t : tkb.genericTripleQuery(
      tkb.getResource(id),
      tkb.getProperty(KBUtils.RDF + "type"),
      null
    )) {
  ...
  private ArrayList<KBObject> getAllCachedClassesOfInstance(
    String id,
    boolean direct
  ) {
    String key = id + ":" + direct;
    if (classcache.containsKey(key)) return classcache.get(key);
    ArrayList<KBObject> clses = kb.getAllClassesOfInstance(
      kb.getResource(id),
      direct
    );
  ...
  protected boolean checkTypeCompatibility(
    KBAPI tkb,
    String varid,
    String argid
  ) {
    ...
    ArrayList<KBObject> argclses =
      this.getAllCachedClassesOfInstance(argid, true);
      ...
      if (argcls.getNamespace().equals(this.dcdomns)) {
        for (KBObject varcls : varclses) {
          if (varcls.getNamespace().equals(this.dcdomns)) {
            if (
              !kb.hasSubClass(argcls, varcls) && !kb.hasSubClass(varcls, argcls)
            ) return false;
          }
        }
      }
      ...
      if (argcls.getNamespace().equals(this.dcdomns)) {
        for (String varclassid : varclassids) {
          ...
          if (varcl.getNamespace().equals(this.dcdomns)) {
            if (
              !kb.hasSubClass(argcls, varcl) && !kb.hasSubClass(varcl, argcls)
            ) return false;
          }
        }
      }
  ...
  public ArrayList<ComponentPacket> specializeAndFindDataDetails(
    ComponentPacket details
  ) {
  ...
  public ComponentPacket findDataDetails(ComponentPacket details) {
    ArrayList<ComponentPacket> list = findDataDetails(
      details,
      false,
      this.useRules
    );
   ...
   * @return
   */
  public ArrayList<ComponentPacket> findDataDetails(
    ComponentPacket details,
    boolean specialize,
    boolean useRules
  ) {
    ...
    if (!cb.isSet()) cbindings.add(cb.getID()); else {
      for (WingsSet s : cb) {
    ...
    try {
      ...
      for (Component ccomp : ccomps) {
        ...
        ArrayList<ComponentRole> allArgs = new ArrayList<ComponentRole>(
          ccomp.getInputs()
        );
        ...
        if (specialize) concreteComponent.setConcrete(
          true
        ...
        for (ComponentRole arg : allArgs) {
          ...
          if (var == null) {
            ...
            short type = 0;
            if (arg.isParam()) type = VariableType.PARAM; else type =
              VariableType.DATA;
            ...
            if (!checkTypeCompatibility(tkb, varid, arg.getID())) {
              logger.debug(
                arg.getID() + " is not type compatible with variable: " + varid
              );
              explanations.add(
                "INFO " +
                ccomp +
                " is not selectable because " +
                arg.getID() +
                " is not type compatible with variable: " +
                varid
              );
              typesOk = false;
              break;
          ...
          tkb.addTriple(
            varobj,
            dmap.get("hasArgumentID"),
            tkb.createLiteral(argid)
          );
        ...
        for (Variable var : roleMaps.values()) {
          if (var.isParameterVariable() && var.getBinding() == null) {
            ...
            ArrayList<KBObject> vals = tkb.getPropertyValues(
              varobj,
              dmap.get("hasValue")
            );
            if (vals != null && vals.size() > 0) {
              ...
              for (KBObject val : vals) {
                ...
                tkb.addTriple(
                  varobj,
                  tkb.getResource(this.wflowns + "hasParameterValue"),
                  val
                );
              }
              var.setBinding(b.size() > 1 ? b : (ValueBinding) b.get(0));
            }
          }
        }
        ...
        for (String id : blacklistedIds) cons.addBlacklistedId(id);
        ArrayList<KBTriple> constraints = cons.getConstraints(varids);
        for (String id : blacklistedIds) cons.removeBlacklistedId(id);
  ...
  private KBRuleList getCachedComponentRules(Component comp) {
    if (rulescache.containsKey(comp.getID())) return rulescache.get(
      comp.getID()
    );
    String rulestr = "";
    for (String str : comp.getRules()) rulestr += str + "\n";
   ...
   */
  public ArrayList<ComponentPacket> findOutputDataPredictedDescriptions(
    ComponentPacket details
  ) {
    ArrayList<ComponentPacket> list = new ArrayList<ComponentPacket>();
    ...
    if (comp == null) {
      ...
      details.addExplanations(
        c.getBinding().getID() + " is not a valid component"
      );
      details.setInvalidFlag(true);
      list.add(details);
      return list;
    ...
    for (ComponentRole role : comp.getInputs()) {
      ...
      if (role.isParam()) {
        ...
        ArrayList<Metric> vartypes = v
          .getBinding()
          .getMetrics()
          .getMetrics()
          .get(KBUtils.RDF + "type");
        if (vartypes != null) {
          ...
          if (!checkTypeCompatibility(varclassids, role.getID())) {
            details.addExplanations(
              ...
              " is not selectable because " +
              role.getID() +
              " is not type compatible with variable binding: " +
              v.getBinding()
            );
            typesOk = false;
            break;
    ...
    try {
      ...
      for (String rolestr : sRoleMap.keySet()) {
        ...
        if (arg == null) {
          details.addExplanations(
            "ERROR Component catalog cannot recognize role id " + rolestr
          );
          continue;
        }
        ...
        if (var.isDataVariable()) {
          ...
          if (var.getBinding() != null) {
            ...
            HashMap<String, ArrayList<Metric>> propValMap =
              metrics.getMetrics();
              ...
              for (Metric tmp : propValMap.get(propid)) {
                ...
                if (metricProp != null) {
                  ...
                  if (type == Metric.URI) {
                    ...
                    // Literal value
                    KBObject tobj = dtype != null
                      ? tkb.createXSDLiteral(valstring, dtype)
                    ...
                    if (tobj != null) {
                      ...
                      details.addExplanations(
                        "ERROR Cannot Convert Metrics Value " + valstring
                      );
                      continue;
                    }
                  }
                  ...
                  details.addExplanations(
                    "ERROR No Such Metrics Property Known to Component Catalog : " +
                    propid
                  );
                  continue;
                }
            ...
            if (var.getBinding().isSet()) {
              ...
              tkb.setPropertyValue(
                varobj,
                dmap.get("hasDimensionSizes"),
                tkb.createLiteral(dimensionSizes)
              );
            }

            if (var.getBinding().getID() != null) tkb.addTriple(
              varobj,
              dmap.get("hasBindingID"),
              tkb.createLiteral(var.getBinding().getName())
            ); else tkb.addTriple(
          ...
          if (parambinding != null && parambinding.getValue() != null) {
            ...
            KBObject arg_value = tkb.createXSDLiteral(
              parambinding.getValueAsString(),
              parambinding.getDatatype()
            );
            tkb.setPropertyValue(varobj, dmap.get("hasValue"), arg_value);
          }
          if (dmap.containsKey("hasBindingID")) // Set the hasBindingID term
          tkb.addTriple(
            varobj,
            dmap.get("hasBindingID"),
      ...
      for (String rolestr : sRoleMap.keySet()) {
        ...
        if (var.isDataVariable() && !sInputRoles.containsKey(rolestr)) {
          ...
          for (KBObject metricProp : metricProps) {
            ...
            // Add value
            if (val.isLiteral()) metrics.addMetric(
              metricProp.getID(),
              new Metric(Metric.LITERAL, val.getValue(), val.getDataType())
            ); else metrics.addMetric(
              metricProp.getID(),
      ...
      if (isInvalid != null && (Boolean) isInvalid.getValue()) {
        details.addExplanations(
          "INFO " + tcomp + " is not valid for its inputs"
        );
      ...
      if (isNoOperation != null && (Boolean) isNoOperation.getValue()) {
        ...
        for (ComponentRole outrole : comp.getOutputs()) {
          ...
          for (KBObject clsobj : this.getAllCachedClassesOfInstance(
              outrole.getID(),
              true
            )) {
          ...
          if (matchingInputs.size() > 0) {
            ...
            if (matchingInputs.size() > 1) {
              for (ComponentRole mInput : matchingInputs) {
                int inPrefix = Integer.parseInt(
                  mInput.getPrefix().substring(2)
                );
          ...
          if (matchingInput != null) {
            ...
            details.addNoOperationIOPassthrough(
              outrole.getRoleName(),
              matchingInput.getRoleName()
            );

            Variable invar = sRoleMap.get(matchingInput.getRoleName());
            logger.debug(
              "Passing through " + invar.getName() + " to " + outvar.getName()
            );
          }
        }
      }
      ...
      if (req != null) {
        if (req.getMemoryGB() != 0) c
          .getRequirements()
          .setMemoryGB(req.getMemoryGB());
        if (req.getStorageGB() != 0) c
          .getRequirements()
          .setStorageGB(req.getStorageGB());
      }
        ...
        if (var.isDataVariable() && !sInputRoles.containsKey(rolestr)) {
          ...
          for (KBObject metricProp : metricProps) {
            ...
            for (KBObject val : vals) {
              if (vals.size() > 1) {
                if (
                  !curmetrics.getMetrics().containsKey(metricProp.getID())
                ) continue;
                // If multiple values present, ignore value that is equal to current value
                for (Metric mval : curmetrics
                  .getMetrics()
                  .get(metricProp.getID())) {
                  if (
                    !val.isLiteral() && val.getID().equals(mval.getValue())
                  ) continue; else if (
                    val.isLiteral() && val.getValue().equals(mval.getValue())
                  ) continue;
                }
              }
              // Add value
          ...
          ArrayList<KBObject> clses =
            this.getAllClassesOfInstance(tkb, varobj.getID());
          for (KBObject cls : clses) metrics.addMetric(
            KBUtils.RDF + "type",
            new Metric(Metric.URI, cls.getID())
          );

          // Set metrics for the Binding
          if (var.getBinding() != null) var.getBinding().setMetrics(metrics);
          ...
          KBObject dimSizesObj = tkb.getPropertyValue(
            varobj,
            dmap.get("hasDimensionSizes")
          );
          KBObject dimIndexPropsObj = tkb.getPropertyValue(
            varobj,
          ...
          if (dim > 0) {
            ...
            while (!vbs.isEmpty()) {
              ...
              for (vdim = 0; vdim < dim; vdim++) {
                if (counter < dimCounters[vdim]) break;
              }
              if (vdim < dim) {
                for (int i = 0; i < dimSizes[vdim]; i++) {
                  Binding cvb = new Binding(
                    b.getNamespace() + UuidGen.generateAUuid("" + i)
                  );
    ...
    try {
      ...
      if (exepath != null) {
        ...
        if (f.isDirectory()) {
          ...
          if (SystemUtils.IS_OS_WINDOWS && winexef.exists()) exepath =
            winexef.getAbsolutePath(); else exepath = shexef.getAbsolutePath();
        }
      }
      ...
      ArrayList<KBObject> inputs =
        this.kb.getPropertyValues(comp, omap.get("hasInput"));
      ArrayList<KBObject> outputs =
        this.kb.getPropertyValues(comp, omap.get("hasOutput"));
      ArrayList<KBObject> args = new ArrayList<KBObject>(inputs);
      args.addAll(outputs);

      for (KBObject arg : args) {
        KBObject argid =
          this.kb.getDatatypePropertyValue(arg, dmap.get("hasArgumentID"));
        String role = (String) argid.getValue();
        for (Role r : roleMap.keySet()) {
          Variable var = roleMap.get(r);
          if (r.getRoleId().equals(role)) setInvocationArguments(
            invocation,
            arg,
            var,
            inputs.contains(arg)
  ...
  private void setInvocationArguments(
    ...
    KBObject prefixobj =
      this.kb.getPropertyValue(arg, dmap.get("hasArgumentName"));
        ...
        if (var.isDataVariable()) {
          ...
          if (var.getBinding() != null) {
            invocation.addArgument(
              prefix,
              ((ValueBinding) var.getBinding()).getValueAsString(),
              var.getID(),
              isInput
            );
