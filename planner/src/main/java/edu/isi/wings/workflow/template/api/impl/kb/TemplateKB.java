...
public class TemplateKB extends URIEntity implements Template, TransactionsAPI {
  ...
  protected void initializeKB(Properties props, boolean load_template) {
    ...
    try {
      ...
      if (load_template) kb =
        this.ontologyFactory.getKB(this.getURL(), OntSpec.PLAIN); else kb =
        this.ontologyFactory.getKB(OntSpec.PLAIN);
  ...
  protected void registerBlankTemplate() {
    this.start_write();
    kb.createObjectOfClass(
      this.getID(),
      kb.getConcept(wflowns + "WorkflowTemplate")
    );
    this.end();
  }
  ...
  public TemplateKB(TemplateKB t) {
    ...
    // copy rules
    this.rules = new Rules();
    if (t.rules != null) this.rules.setRulesText(t.rules.getRulesText());
    ...
    if (t.metadata != null) {
      ...
      this.metadata.contributors =
        new ArrayList<String>(t.metadata.contributors);
    }
    ...
    this.constraintEngine =
      new ConstraintEngineKB((ConstraintEngineKB) t.getConstraintEngine());
  }
  ...
  private void cacheConceptsAndProperties() {
    ...
    for (KBObject obj : kb.getAllClasses()) {
      if (obj != null) conceptObjMap.put(obj.getName(), obj);
    }
    for (KBObject obj : kb.getAllObjectProperties()) {
      if (obj != null) propertyObjMap.put(obj.getName(), obj);
    }
    ...
    if (!propertyObjMap.containsKey("isSkipped")) {
      ...
      propertyObjMap.put(
        "isSkipped",
        kb.createDatatypeProperty(this.wflowns + "isSkipped")
      );
      this.end();
    }
  }
  ...
  private Node readNodeFromKB(KBObject obj) {
    ...
    KBObject compObj = kb.getPropertyValue(
      obj,
      propertyObjMap.get("hasComponent")
    );
    KBObject wObj = kb.getPropertyValue(obj, propertyObjMap.get("hasWorkflow"));
    KBObject isInactive = kb.getPropertyValue(
      obj,
      propertyObjMap.get("isInactive")
    );
    KBObject isSkip = kb.getPropertyValue(obj, propertyObjMap.get("isSkipped"));

    Node n = new Node(obj.getID());

    if (isInactive != null && (Boolean) isInactive.getValue()) n.setInactive(
      true
    );

    if (isSkip != null && (Boolean) isSkip.getValue()) n.setSkip(true);

    if (compObj != null) {
      ComponentVariable comp = new ComponentVariable(compObj.getID());

      KBObject cval = kb.getDatatypePropertyValue(
        compObj,
        propertyObjMap.get("isConcrete")
      );
      if (
        cval != null && cval.getValue() != null && (Boolean) cval.getValue()
      ) comp.setConcrete(true);

      KBObject cbinding = kb.getPropertyValue(
        compObj,
      ...
      if (cbinding != null) {
        ...
        ArrayList<KBObject> compClses = kb.getPropertyValues(
          compObj,
          kb.getProperty(KBUtils.RDF + "type")
        );
        for (KBObject compCls : compClses) {
          if (!compCls.getNamespace().equals(this.wflowns)) comp.setBinding(
            new Binding(compCls.getID())
          );
        }
      }
    ...
    for (KBObject mobj : kb.getPropertyValues(
      obj,
      propertyObjMap.get("canRunOn")
    )) {
      machineIds.add(mobj.getID());
    }
    n.setMachineIds(machineIds);

    KBObject dnode = kb.getPropertyValue(
      obj,
      propertyObjMap.get("derivedFrom")
    );
    if (dnode != null) n.setDerivedFrom(dnode.getID());
  ...
  protected void readTemplate() {
    ...
    ArrayList<KBObject> tobjs = kb.getInstancesOfClass(
      conceptObjMap.get("WorkflowTemplate"),
      false
    );
    ...
    for (String id : subtemplates.keySet()) {
      ...
      if (t.getParent() == null) {
        ...
        for (Link l : t.getLinks()) this.addLink(l);
        for (Variable v : t.getVariables()) this.addVariable(v);
        for (Node n : t.getNodes()) this.addNode(n);
  ...
  private Template readTemplate(KBObject templateObj) {
    if (subtemplates.containsKey(templateObj.getID())) return subtemplates.get(
      templateObj.getID()
    );
    ...
    if (verobj != null) {
      if (verobj.getValue().getClass() == Float.class) t.version =
        ((Float) verobj.getValue()).intValue(); else if (
        verobj.getValue().getClass() == Integer.class
      ) t.version = (Integer) verobj.getValue();
    }

    // ns = (String) kb.getPrefixNamespaceMap().get("");

    ArrayList<KBObject> linkObjs = kb.getPropertyValues(
      templateObj,
      pmap.get("hasLink")
    );
    ArrayList<KBObject> nodeObjs = kb.getPropertyValues(
      templateObj,
      pmap.get("hasNode")
    );
    ...
    for (KBObject nodeObj : nodeObjs) {
      ...
      /* New Version Queries */
      if (t.version > 0) {
        ArrayList<KBObject> ports = kb.getPropertyValues(
          nodeObj,
          pmap.get("hasInputPort")
        );
        ...
        KBObject cruleobj = kb.getPropertyValue(
          nodeObj,
          pmap.get("hasComponentSetCreationRule")
        );
        KBObject pruleobj = kb.getPropertyValue(
          nodeObj,
        ...
        if (cruleobj != null) {
          ...
          KBObject stype = kb.getPropertyValue(
            cruleobj,
            pmap.get("createComponentSets")
          );
          KBObject wtype = kb.getPropertyValue(
            cruleobj,
            pmap.get("createWorkflowSets")
          );
        ...
        if (pruleobj != null) {
          KBObject exprobj = kb.getPropertyValue(
            pruleobj,
            pmap.get("createSetsOn")
          );
        ...
        if (crule != null) node.addComponentSetRule(crule);
        if (prule != null) node.addPortSetRule(prule);
      } else {
    ...
    for (KBObject linkObj : linkObjs) {
      Node fromNode = getNode(
        kb.getPropertyValue(linkObj, pmap.get("hasOriginNode")),
        t.Nodes
      );
      Node toNode = getNode(
        kb.getPropertyValue(linkObj, pmap.get("hasDestinationNode")),
        t.Nodes
      );
      ...
      if (t.version > 0) {
        KBObject fromPortObj = kb.getPropertyValue(
          linkObj,
          pmap.get("hasOriginPort")
        );
        KBObject toPortObj = kb.getPropertyValue(
          linkObj,
          pmap.get("hasDestinationPort")
        );
        ...
        KBObject fromParamObj = kb.getPropertyValue(
          linkObj,
          pmap.get("hasOriginParameter")
        );
        KBObject toParamObj = kb.getPropertyValue(
          linkObj,
      ...
      if (var == null) {
        if (kb.isA(varObj, cmap.get("DataVariable"))) {
          ...
          KBObject dsBinding = kb.getPropertyValue(
            varObj,
            pmap.get("hasDataBinding")
          );
          if (dsBinding != null) {
            var.setBinding(readBindingObjectFromKB(kb, dsBinding));
          }
          KBObject breakPoint = kb.getPropertyValue(
            varObj,
            pmap.get("breakPoint")
          );
          if (
            breakPoint != null && (Boolean) breakPoint.getValue()
          ) var.setBreakpoint(true);
        } else if (kb.isA(varObj, cmap.get("ParameterVariable"))) {
          var = new ParameterVariable(varObj.getID());
          KBObject paramValue = kb.getDatatypePropertyValue(
            varObj,
        ...
        KBObject dvar = kb.getPropertyValue(
          varObj,
          propertyObjMap.get("derivedFrom")
        );
        if (dvar != null) var.setDerivedFrom(dvar.getID());

        KBObject autoFill = kb.getPropertyValue(varObj, pmap.get("autoFill"));
        if (autoFill != null && (Boolean) autoFill.getValue()) var.setAutoFill(
          true
        );
      }
      if (var != null) {
        String comment = kb.getComment(varObj);
        if (comment != null) var.setComment(comment);
        t.addVariable(var);

        // Set Role type from variable type
        if (fromPort != null) fromPort.getRole().setType(var.getVariableType());
        if (toPort != null) toPort.getRole().setType(var.getVariableType());

        String lid = linkObj.getID();
        if (lid == null) lid = this.createLinkId(fromPort, toPort, var);
  ...
  private void readTemplateRolesFromKB(
    KBAPI kb,
    Template t,
    KBObject templateObj
  ) {
    ArrayList<KBObject> iroleObjs = kb.getPropertyValues(
      templateObj,
      propertyObjMap.get("hasInputRole")
    );
    ArrayList<KBObject> oroleObjs = kb.getPropertyValues(
      templateObj,
    ...
    for (KBObject iroleObj : iroleObjs) {
      ...
      KBObject varObj = kb.getPropertyValue(
        iroleObj,
        propertyObjMap.get("mapsToVariable")
      );
      KBObject dimObj = kb.getDatatypePropertyValue(
        iroleObj,
        propertyObjMap.get("hasDimensionality")
      );
      KBObject idObj = kb.getDatatypePropertyValue(
        iroleObj,
        propertyObjMap.get("hasRoleID")
      );
      if (varObj == null) {
        System.err.println(
          "Warning: Role " + iroleObj + " not mapped to any variable"
        );
        continue;
      }
      // Set Role type from variable type
      ...
      if (var == null) {
        System.err.println(
          "Warning: Role " + r + " mapped to non existent variable " + varObj
        );
        continue;
      }
    ...
    for (KBObject oroleObj : oroleObjs) {
      ...
      if (varObj == null) {
        System.err.println(
          "Warning: Role " + oroleObj + " not mapped to any variable"
        );
        continue;
      }
      // Set Role type from variable type
  ...
  private void addDefaultSetCreationRulesForNode(Node n) {
    ...
    if (prule.getSetExpression() == null) prule =
      new PortSetCreationRule(
        prule.getType(),
        new SetExpression(SetOperator.XPRODUCT)
      );
    ...
    while (!exprs.isEmpty()) {
      ...
      if (ex == null) continue;
      if (ex.isSet()) exprs.addAll(ex); else ruleports.add(ex.getPort());
    }
    ...
    for (Link l : links) {
      ...
      if (l.getVariable() != null && !ruleports.contains(p)) prule
        .getSetExpression()
        .add(new SetExpression(SetOperator.XPRODUCT, p));
    }
  ...
  protected Metadata readMetadata(KBAPI kb, KBObject tobj) {
    ...
    KBObject mobj = kb.getPropertyValue(
      tobj,
      this.propertyObjMap.get("hasMetadata")
    );
    if (mobj == null) return m;

    // Fetch metadata
    KBObject val = kb.getPropertyValue(
      mobj,
      propertyObjMap.get("lastUpdateTime")
    );
    if (val != null) {
    ...
    if (val != null) m.documentation = (String) val.getValue();

    val = kb.getPropertyValue(mobj, propertyObjMap.get("tellmeData"));
    if (val != null) m.tellme = (String) val.getValue();

    ArrayList<KBObject> crs = kb.getPropertyValues(
      mobj,
      propertyObjMap.get("hasContributor")
    );
    for (KBObject cr : crs) {
      if (!m.contributors.contains(cr.getValue())) m.contributors.add(
        (String) cr.getValue()
      );
    }

    ArrayList<KBObject> templates = kb.getPropertyValues(
      mobj,
      propertyObjMap.get("createdFrom")
    );
    for (KBObject tmp : templates) {
      if (!m.createdFrom.contains(tmp.getValue())) m.createdFrom.add(
        (String) tmp.getValue()
      );
    }
  ...
  protected Rules readRules(KBAPI kb, KBObject tobj) {
    ...
    Rules r = new Rules();
    KBObject robj = kb.getPropertyValue(
      tobj,
      this.propertyObjMap.get("hasRules")
    );
    if (robj == null) return r;

    // Fetch rules
    KBObject val = kb.getPropertyValue(robj, propertyObjMap.get("hasRules"));
    if (val != null) r.setRulesText((String) val.getValue());
  ...
  private Port readPortFromKB(KBObject portObj) {
    ...
    KBObject roleObj = kb.getPropertyValue(
      portObj,
      propertyObjMap.get("satisfiesRole")
    );
    if (roleObj != null) {
      ...
      if (!role.getNamespace().equals(this.getNamespace())) {
        ...
        KBObject roleid = kb.getDatatypePropertyValue(
          roleObj,
          propertyObjMap.get("hasRoleID")
        );
        if (roleid != null) role.setRoleId((String) roleid.getValue());
      }
      // Get Role Dimensionality
      KBObject dim = kb.getDatatypePropertyValue(
        roleObj,
        propertyObjMap.get("hasDimensionality")
      );
      if (dim != null && dim.getValue() != null) role.setDimensionality(
  ...
  public Link[] getInputLinks(Node n) {
    if (
      this.nodeInputLinks.containsKey(n.getID())
  ...
  public Variable[] getInputDataVariables() {
    ...
    for (int i = 0; i < links.length; i++) {
      Variable var = links[i].getVariable();
      if (var.isDataVariable()) varAr.add(var);
    }
    return varAr.toArray(new Variable[varAr.size()]);
  }
  ...
  public Link getLink(Node fromN, Node toN, Port fromPort, Port toPort) {
    ...
    if (fromN != null && nodeOutputLinks.containsKey(fromN.getID())) {
      ...
      if (
        toN != null && nodeInputLinks.containsKey(toN.getID())
    ...
    for (Link l : links) {
      boolean ok = true;
      if (
        l.getOriginPort() != null &&
        fromPort != null &&
        !l.getOriginPort().getID().equals(fromPort.getID())
      ) ok = false;
      if (
        l.getDestinationPort() != null &&
        toPort != null &&
        !l.getDestinationPort().getID().equals(toPort.getID())
      ) ok = false;
      if (ok) return l;
    }
    return null;
  ...
  public Link[] getLinks(Node fromN, Node toN) {
    ArrayList<Link> links = new ArrayList<Link>(
      this.nodeOutputLinks.get(fromN.getID())
    );
    return links.toArray(new Link[0]);
  }
  ...
  public ComponentVariable getComponentVariable(String cid) {
    for (Node n : Nodes.values()) {
      if (
        n.getComponentVariable().getID().equals(cid)
  ...
  public Link[] getOutputLinks(Node n) {
    if (
      this.nodeOutputLinks.containsKey(n.getID())
    ) return this.nodeOutputLinks.get(n.getID())
      .toArray(new Link[0]); else return new Link[] {};
  }
  ...
  private String createLinkId(Port fromPort, Port toPort, Variable var) {
    ...
    if (fromPort != null) lid += fromPort.getName() + "_To"; else lid += "To";

    if (toPort != null) lid += "_" + toPort.getName(); else lid += "_Output";

    return lid;
  }

  public void updateLinkDetails(Link l) {
    if (this.nodeInputLinks == null) this.initMaps();
  ...
  private Link addLink(
    ...
    if (
      fromN != null && fromN.findInputPort(fromPort.getID()) == null
    ) fromN.addOutputPort(fromPort);

    if (var != null) {
      if (getVariable(var.getID()) == null) addVariable(var);
      l.setVariable(var);
    }
    this.addLinkMaps(l);
    Links.put(lid, l);
  ...
  public void deleteLink(Link l) {
    ...
    if (l.getDestinationNode() != null) l
      .getDestinationNode()
      .deleteInputPort(l.getDestinationPort());
    if (l.getOriginNode() != null) l
      .getOriginNode()
      .deleteOutputPort(l.getOriginPort());
  }
  ...
  public Variable addVariable(
    ...
    String vid = varid;
    int i = 1;
    if (isCollectionItem && type != VariableType.PARAM) vid =
      varid + "_" + String.format("%04d", i++);

    while (getVariable(vid) != null) {
      vid = varid + "_" + String.format("%04d", i++);
    }

    Variable v = type == VariableType.PARAM
      ? new ParameterVariable(vid)
      : new DataVariable(vid);
    this.addVariable(v);
    return v;
  }
  ...
  private ValueBinding copyTemplateBindings(ValueBinding b) {
    if (b.isSet()) {
      ...
      for (WingsSet bb : b) rb.add(copyTemplateBindings((ValueBinding) bb));
      return rb;
    } else return new ValueBinding(((Template) b.getValue()).createCopy());
  }
  ...
  public Template createCopy() {
    ...
    for (Node e : Nodes.values()) {
      ...
      if (ev.getBinding() != null) {
        if (ev.isTemplate()) {
          ...
          cv.setBinding(
            (Binding) SerializableObjectCloner.clone(ev.getBinding())
          );
          if (ev.getDerivedFrom() != null) cv.setDerivedFrom(
            ev.getDerivedFrom()
          ); else cv.setDerivedFrom(ev.getID());
        }
      }
      ...
      if (e.getDerivedFrom() != null) n.setDerivedFrom(
        e.getDerivedFrom()
      ); else n.setDerivedFrom(e.getID());
      ...
      if (crule != null) {
        ComponentSetCreationRule ncrule = new ComponentSetCreationRule(
          crule.getType()
        );
      ...
      if (prule != null) {
        ...
        PortSetCreationRule nprule = new PortSetCreationRule(
          prule.getType(),
          nexpr
        );
      ...
      if (vv == null) {
        if (v.isDataVariable()) {
          ...
          if (v.getBinding() != null) {
            vv.setBinding(
              (Binding) SerializableObjectCloner.clone(v.getBinding())
            );
          }
    ...
    t
      .getConstraintEngine()
      .addConstraints(this.constraintEngine.getConstraints(varids));
  ...
  public KBAPI getKBCopy(boolean includeDataConstraints) {
    ...
    KBAPI tkb = ontologyFactory.getKB(
      new ByteArrayInputStream(getInternalRepresentation().getBytes()),
      this.getNamespace(),
      OntSpec.PLAIN
    );
  ...
  public boolean saveAs(String newid) {
    try {
      ...
      // Set derivedFrom to the original ids of all nodes and variables (if not already set)
      for (Node n : this.Nodes.values()) {
        if (n.getDerivedFrom() == null) n.setDerivedFrom(n.getID());
      }
      for (Variable v : this.Variables.values()) {
        if (v.getDerivedFrom() == null) v.setDerivedFrom(v.getID());
      }
  ...
  public boolean delete() {
    return this.start_write() && kb.delete() && this.save(kb) && this.end();
  }
  ...
  private ValueBinding readValueBindingObjectFromKB(KBAPI tkb, KBObject bobj) {
    ValueBinding b = new ValueBinding(
      this.getNamespace() + UuidGen.generateAUuid("")
    );
    if (bobj.isList()) {
      ArrayList<KBObject> items = tkb.getListItems(bobj);
      for (KBObject item : items) {
  ...
  private KBObject writeBindingObjectToKB(KBAPI tkb, Binding b) {
    if (!b.isSet()) return tkb.getResource(b.getID());
  ...
  private KBObject writeValueBindingObjectToKB(KBAPI tkb, ValueBinding b) {
    if (!b.isSet()) {
      if (b.getDatatype() != null) return tkb.createXSDLiteral(
        b.getValueAsString(),
        b.getDatatype()
      ); else return tkb.createLiteral(b.getValue());
    }
  ...
  private KBAPI serializeIntoKB(KBAPI tkb, boolean subtemplate) {
    ...
    // Create a template
    KBObject tobj = tkb.createObjectOfClass(
      this.getID(),
      cmap.get("WorkflowTemplate")
    );
    ...
    for (Node n : Nodes.values()) {
      ...
      if (n.getDerivedFrom() != null) tkb.addTriple(
        nobj,
        propertyObjMap.get("derivedFrom"),
      ...
      if (c != null && !c.isTemplate()) {
        ...
        if (cobj == null) cobj =
          tkb.createObjectOfClass(c.getID(), cmap.get("ComponentVariable"));

        if (cobj != null) {
          tkb.addPropertyValue(nobj, pmap.get("hasComponent"), cobj);
          if (c.isConcrete()) tkb.addPropertyValue(
            cobj,
            pmap.get("isConcrete"),
            tkb.createLiteral(true)
          );
      ...
      if (crule != null) {
        KBObject cruleobj = tkb.createObjectOfClass(
          n.getID() + "_crule",
          cmap.get("ComponentSetRule")
        );
      ...
      if (prule != null) {
        KBObject pruleobj = tkb.createObjectOfClass(
          n.getID() + "_prule",
          cmap.get("PortSetRule")
        );
        ...
        if (expr != null) {
          ...
          if (exprobj != null) tkb.addPropertyValue(
            pruleobj,
            pmap.get("createSetsOn"),
            exprobj
          );
    ...
    for (Variable v : Variables.values()) {
      ...
      if (v.isDataVariable()) {
        ...
        vobj =
          tkb.createObjectOfClass(v.getID(), cmap.get("ParameterVariable"));
        if (v.getBinding() != null) {
          ArrayList<ValueBinding> valueBindings = new ArrayList<ValueBinding>();
          if (!v.getBinding().isSet()) valueBindings.add(
            (ValueBinding) v.getBinding()
          ); else {
      ...
      if (v.getDerivedFrom() != null) tkb.addPropertyValue(
        vobj,
        propertyObjMap.get("derivedFrom"),
    ...
    for (Link l : Links.values()) {
      KBObject lc = null;
      if (l.isInputLink()) lc = cmap.get("InputLink"); else if (
        l.isInOutLink()
      ) lc = cmap.get("InOutLink"); else if (l.isOutputLink()) lc =
        cmap.get("OutputLink");
      if (lc != null) {
        ...
        if (on != null) tkb.addPropertyValue(
          lobj,
          pmap.get("hasOriginNode"),
          tkb.getResource(on.getID())
        );
        if (ocp != null) tkb.addPropertyValue(
          lobj,
          pmap.get("hasOriginPort"),
          tkb.getResource(ocp.getID())
        );
        if (dn != null) tkb.addPropertyValue(
          lobj,
          pmap.get("hasDestinationNode"),
          tkb.getResource(dn.getID())
        );
        if (dcp != null) tkb.addPropertyValue(
          lobj,
          pmap.get("hasDestinationPort"),
          tkb.getResource(dcp.getID())
        );
        if (v != null) tkb.addPropertyValue(
          lobj,
          pmap.get("hasVariable"),
    ...
    if (!subtemplate) {
      ...
      if (metadata != null) writeMetadataDescription(tkb, tobj, metadata);

      if (rules != null) writeRules(tkb, tobj, rules);
    }
  ...
  private void writeTemplateRolesInKB(KBAPI tkb, KBObject templateObj) {
    for (String varid : inputRoles.keySet()) {
      Role r = inputRoles.get(varid);
      KBObject roleobj = tkb.createObjectOfClass(
        r.getID(),
        conceptObjMap.get("Role")
      );
      tkb.addPropertyValue(
        templateObj,
        propertyObjMap.get("hasInputRole"),
  ...
  private SetExpression readSetExpressionFromKB(
    KBAPI tkb,
    KBObject exprobj,
    Node n
  ) {
    ...
    if (exprcls.getID().equals(conceptObjMap.get("XProduct").getID())) {
      ...
      exprcls
        .getID()
        .equals(conceptObjMap.get("IncreaseDimensionality").getID())
    ) {
      expr = new SetExpression(SetOperator.INCREASEDIM);
    } else if (
      exprcls.getID().equals(conceptObjMap.get("ReduceDimensionality").getID())
    ) {
    ...
    if (!isleaf) {
      ArrayList<KBObject> argobjs = tkb.getPropertyValues(
        exprobj,
        propertyObjMap.get("hasExpressionArgument")
      );
      if (argobjs != null) {
  ...
  private SetExpression copySetExpression(Node n, SetExpression expr) {
    if (expr.isSet()) {
      ...
      for (SetExpression cexpr : expr) nexpr.add(copySetExpression(n, cexpr));
      return nexpr;
    } else {
      return new SetExpression(
        expr.getOperator(),
        n.findInputPort(expr.getPort().getID())
      );
    }
  }

  protected void writeMetadataDescription(
    KBAPI tkb,
    KBObject tobj,
    Metadata m
  ) {
    // Add metadata
    // this.cacheConceptsAndProperties();
    KBObject mobj = tkb.createObjectOfClass(
      this.getID() + "_meta",
      conceptObjMap.get("Metadata")
    );
    tkb.setPropertyValue(tobj, propertyObjMap.get("hasMetadata"), mobj);

    if (m.lastUpdateTime != null) tkb.setPropertyValue(
      mobj,
      propertyObjMap.get("lastUpdateTime"),
      tkb.createLiteral(m.getLastUpdateTime())
    );

    if (m.documentation != null) tkb.setPropertyValue(
      mobj,
      propertyObjMap.get("hasDocumentation"),
      tkb.createLiteral(m.documentation)
    );

    for (String tmp : m.createdFrom) if (tmp != null) tkb.addPropertyValue(
      mobj,
      propertyObjMap.get("createdFrom"),
      tkb.createLiteral(tmp)
    );
    for (String tmp : m.contributors) if (tmp != null) tkb.addPropertyValue(
      mobj,
      propertyObjMap.get("hasContributor"),
      tkb.createLiteral(tmp)
    );

    if (m.tellme != null) tkb.setPropertyValue(
      mobj,
      propertyObjMap.get("tellmeData"),
  ...
  protected void writeRules(KBAPI tkb, KBObject tobj, Rules rules) {
    // Add rules
    // this.cacheConceptsAndProperties();
    if (rules.getRulesText() != null) tkb.setPropertyValue(
      tobj,
      propertyObjMap.get("hasRules"),
  ...
  public Template applyRules() {
    ...
    ArrayList<KBObject> templates = tkb.getInstancesOfClass(
      tkb.getConcept(ns + "WorkflowTemplate"),
      false
    );
  ...
  public void autoLayout() {
    ...
    if (this.getLinks().length > MAX_LINKS) return;

    String dotexe = this.props.getProperty("dot.path");
    if (dotexe == null) return;
    ...
    for (Node n : this.Nodes.values()) {
      ...
      for (Port p : n.getInputPorts()) ips.add(cleanID(p.getName()));
      for (Port p : n.getOutputPorts()) ops.add(cleanID(p.getName()));
      Collections.sort(ips);
      Collections.sort(ops);
      int fsize = 13;

      dotstr += nl + tab + nid + "[label=\"{{";
      for (int i = 0; i < ips.size(); i++) dotstr += "|<" + ips.get(i) + ">";
      dotstr += "|}|{" + ntext + "}|{";
      for (int i = 0; i < ops.size(); i++) dotstr += "|<" + ops.get(i) + ">";
    ...
    for (Link l : this.Links.values()) {
      if (l.getOriginPort() != null) {
        String lid =
          cleanID(l.getOriginNode().getName()) +
          ":" +
          cleanID(l.getOriginPort().getName()) +
          " -> " +
          cleanID(l.getVariable().getName()) +
          ":ip;";
  ...
  private HashMap<String, Double[]> getDotLayout(String dotstr, String dotexe)
    throws IOException {
    ...
    BufferedReader bin = new BufferedReader(
      new InputStreamReader(dot.getInputStream())
    );
    ...
    while ((line = bin.readLine()) != null) {
      ...
      if (tmp[0].equals("stop")) break;
      if (tmp.length < 4) continue;
      if (!tmp[0].equals("node")) continue;
      String id = tmp[1];
      ...
      idpositions.put(id, new Double[] { 10 + DPI * x, 30 + DPI * y });
    }
    bin.close();
    bout.close();
    return idpositions;
  ...
  public boolean start_read() {
    if (transaction != null) return transaction.start_read();
    return true;
  }

  @Override
  public boolean start_write() {
    if (transaction != null) return transaction.start_write();
    return true;
  }

  @Override
  public boolean read(Runnable r) {
    if (transaction != null) return transaction.read(r);
    return true;
  }

  @Override
  public boolean write(Runnable r) {
    if (transaction != null) return transaction.write(r);
    return true;
  }
  ...
  @Override
  public boolean end() {
    if (transaction != null) return transaction.end();
    return true;
  }
