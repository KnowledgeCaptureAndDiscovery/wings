...
public class Wings {
  ...
  public Wings(
    ...
    if (isTemplate) this.templateName = requestName; else this.seedName =
      requestName;
  }
  ...
  public void initializeItem() {
    if (this.workOnTemplate) initializeTemplate(); else initializeSeed();
  }
  ...
  private void initializeTemplate() {
    LogEvent event = getEvent(LogEvent.EVENT_WG_LOAD_SEED);
    logger.info(
      event.createStartLogMsg().addWQ(LogEvent.TEMPLATE, templateName)
    );
  ...
  public ArrayList<Template> backwardSweep(Seed seed) {
    ...
    ArrayList<Template> innerSpecialized = wg.specializeTemplates(
      inferredTemplate
    );
    for (Template template : innerSpecialized) {
      ...
      template
        .getMetadata()
        .addCreationSource(seed.getName() + "(Specialized)");
  ...
  public ArrayList<Template> selectInputData(
    ArrayList<Template> candidateWorkflows
  ) {
    ...
    for (Template candidateWorkflow : candidateWorkflows) {
      ...
      VariableBindingsListSet bindingset = wg.selectInputDataObjects(
        candidateWorkflow
      );
      if (bindingset != null) {
    ...
    logger.info(
      event
        ...
        .addWQ(
          LogEvent.MSG,
          "Select Input Data Objects returned " +
          boundWorkflows.size() +
          " templates from " +
          candidateWorkflows.size() +
          " templates."
  ...
  public void writeDataSelections(
  ...
  public ArrayList<Template> getExpandedTemplates(
    ArrayList<Template> configuredTemplates
  ) {
    ...
    for (Template ct : configuredTemplates) {
      ...
      if (expt != null) expansions.add(expt);
    }
    return expansions;
  }
  ...
  public RuntimePlan runPlan(ExecutionPlan plan, boolean waitToComplete) {
    try {
      ...
      if (waitToComplete) rplan.waitFor();
      return rplan;
    } catch (Exception e) {
      e.printStackTrace();
    }
  ...
  public void writeParameterSelections(
    ...
    while (paramBindings_b.size() > 0) {
      ...
      for (String varid : paramBinding_b.keySet()) {
        ...
        if (b.isSet()) {
          ...
          hasSets = true;
        } else paramBinding.put(varid, b.toString());
      }
      if (!hasSets) paramBindings.add(paramBinding);
    }
    try {
      PrintStream out = new PrintStream(new FileOutputStream(file));
  ...
  private ArrayList<Template> randomSelection(
    ...
    int size = items.size();
    logger.info(
      "Pruning search space by randomly choosing " +
      num +
      " out of " +
      size +
      " templates"
  ...
  public static void main(String[] args) {
    ...
    if (options.get("validate") != null) {
      wings.writeTemplateRDF(
        isTemplate ? wings.template : wings.seed,
    ...
    if (options.get("elaborate") != null) {
      Template it = wings.wg.getInferredTemplate(
        isTemplate ? wings.template : wings.seed
      );
      if (it == null) {
        wings.end(ev, 1);
    ...
    if (isTemplate) {
      System.err.println(
        "Error: Seed Required ( with -s ) for desired operation"
      );
      Arguments.displayUsage("Wings");
      System.exit(1);
    }
    ...
    if (options.get("getParameters") != null) {
      ...
      wings.writeParameterSelections(
        configurations,
        options.get("getParameters")
      );
      System.exit(0);
    }
