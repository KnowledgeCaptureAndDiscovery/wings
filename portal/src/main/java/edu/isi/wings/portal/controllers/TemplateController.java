...
public class TemplateController {
  ...
  public String getBeamerParaphrasesJSON() {
    try {
      ...
      return FileUtils.readWholeFileAsUTF8(
        beamerDir + File.separator + "paraphrases.json"
      );
    } catch (Exception e) {
      //e.printStackTrace();
    }
  ...
  public String getBeamerMappingsJSON() {
    try {
      String beamerDir =
        config.getDomain().getDomainDirectory() + File.separator + "beamer";
      return FileUtils.readWholeFileAsUTF8(
        beamerDir + File.separator + "mappings.json"
      );
    } catch (Exception e) {
      //e.printStackTrace();
    }
  ...
  public synchronized String saveTemplateJSON(
    String tplid,
    String templatejson,
    String consjson
  ) {
    ...
    if (
      !tpl.getMetadata().getContributors().contains(this.config.getUserId())
    ) tpl.getMetadata().addContributor(this.config.getUserId());

    if (tpl != null) {
      boolean ok = false;
      if (tplid.equals(tpl.getID())) {
        ok = tpl.save() && this.tc.registerTemplate(tpl);
      } else {
        ok = tpl.saveAs(tplid);
        this.tc.registerTemplateAs(tpl, tplid);
      }

      if (ok && prov.addProvenance(p)) return "OK";
    }
    return "";
  }
  ...
  public String layoutTemplate(String tjson, String dotexe) throws IOException {
    Template tpl = JsonHandler.getTemplateFromJSON(this.json, tjson, "[]");
    tpl.autoLayout();
    return JsonHandler.getTemplateJSON(this.json, tpl, null);
  }

  private ArrayList<HashMap<String, Object>> getTemplateInputs(
    Template tpl,
    boolean dataoptions
  ) {
    ...
    for (Link l : tpl.getInputLinks()) {
      ...
      if (var.isAutoFill()) continue;
      if (varsDone.containsKey(var.getID())) continue;
      ...
      Component comp = compCache.containsKey(cid)
        ? compCache.get(cid)
        : this.cc.getComponent(cid, true);
      ...
      for (ComponentRole crole : comp.getInputs()) {
        if (crole.getType() == null) continue;
      ...
      int dim = varDims.containsKey(var.getID())
        ? varDims.get(var.getID())
        : roledim;
      vardata.put("dim", dim);
      ...
      if (var.getVariableType() == VariableType.DATA) {
        ...
        if (roletypeid != null && dataoptions) {
          ArrayList<DataItem> dataitems =
            this.dc.getDataForDatatype(roletypeid, false);
        ...
        vardata.put(
          "binding",
          (varbinding != null ? varbinding.getValue() : "")
        );
      }
