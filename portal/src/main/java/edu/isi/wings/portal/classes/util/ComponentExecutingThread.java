...
public class ComponentExecutingThread implements Runnable {
  ...
  private ExecutionPlan getExecutionPlan() {
    ...
    for (String roleid : this.role_bindings.keySet()) {
      ...
      Variable v = new Variable(roleid, (b.isValueBinding() ? VariableType.PARAM : VariableType.DATA));
      v.setBinding(b);
      roleMap.put(r, v);
    }
    ComponentPacket mapsComponentDetails = new ComponentPacket(cv, roleMap, new ArrayList<KBTriple>());
    ComponentInvocation invocation = this.api_bindings.cc.getComponentInvocation(mapsComponentDetails);
