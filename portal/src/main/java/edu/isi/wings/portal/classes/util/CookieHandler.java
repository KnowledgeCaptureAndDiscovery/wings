...
public class CookieHandler {
  public static Cookie httpCookieFromServletCookie(javax.servlet.http.Cookie cookie, HttpServletRequest request, Config config) {
    if (cookie == null) {
      return null;
    }
  ...
  public static Cookie[] httpCookiesFromServlet(HttpServletRequest request, Config config) {
    ...
    for(int i=0; i<cookies.length; i++) {
      apacheCookies[i] = httpCookieFromServletCookie(cookies[i], request, config);
    }
