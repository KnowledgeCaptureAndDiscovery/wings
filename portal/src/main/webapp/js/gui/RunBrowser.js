...
RunBrowser.prototype.setupTemplateBrowser = function () {
  return new TemplateBrowser(
    this.guid,
    {
      hide_selector: true,
      hide_form: true,
      hide_intro: true,
      hide_documentation: true,
      hide_title: true,
    },
    null,
    false,
...
RunBrowser.prototype.formatRunListItem = function (
  value,
  meta,
  record,
  rowind,
  ...
  if (rstat == "SUCCESS") icon = "icon-select-alt fa-big fa-green";
  else if (rstat == "FAILURE") icon = "icon-cancel-alt fa-big fa-red";

  var tmpl =
    "<i style='float:left;padding:5px' class='" +
    icon +
    "'></i>" +
    "<div>" +
...
RunBrowser.prototype.formatProgressBar = function (
  value,
  meta,
  record,
  rowind,
  ...
  if (d.status == "FAILURE") tmpl += "<div class='running'>Failed: {8}</div>";
  else if (d.status == "RUNNING")
    tmpl += "<div class='running'>Running: {9}</div>";
  else if (d.status == "SUCCESS")
    tmpl += "<div class='running'>Completed</div>";
  else if (d.status == "QUEUED") tmpl += "<div class='running'>Queued</div>";
  var w = 190;
  ...
  return Ext.String.format(
    ...
    10,
    Math.round(
      (w * (d.percent_done + d.percent_failed + d.percent_running)) / 100
    ),
...
RunBrowser.prototype.formatBinding = function (b, meta, record, runid) {
  var str = "";
  var This = this;
  if (b.type == "literal") str += "<b style='color:green'>" + b.value + "</b>";
  else if (b.size != -1 && b.location) {
    var href = This.data_url + "/fetch?data_id=" + escape(b.id);
    str +=
      "<a href='" + href + "' target='_blank'>" + getLocalName(b.id) + "</a> ";
    str += "<span style='color:#888'>(";
    str += This.formatSize(b.size);
    str += ")</span>";
  } else str += "<b style='color:red'>Not available</b>";
  return str;
};
...
RunBrowser.prototype.getVariableBindingData = function (data) {
  ...
  for (var i = 0; i < steps.length; i++) {
    ...
    for (var j = 0; j < step.inputFiles.length; j++) {
      ...
      for (var key in file.metadata) file.metadata[key] = [file.metadata[key]];
  ...
  var bindingmaps = {};
  var variables = []
    .concat(data.variables.input)
    .concat(data.variables.output)
    .concat(data.variables.intermediate);

  for (var i = 0; i < variables.length; i++) {
    var v = variables[i];
    var binding = filedata[v.id] ? filedata[v.id] : v.binding;
    if (v.binding.id) binding.id = v.binding.id;
  ...
  var varmap = function (variables, type) {
    for (var i = 0; i < variables.length; i++) {
      var v = variables[i];
      var binding = v.binding.id
        ? bindingmaps[v.binding.id]
        : filedata[v.id]
        ? filedata[v.id]
        : v.binding;
      if (v.binding.id) binding.id = v.binding.id;
  ...
  for (var vid in data.constraints) {
    if (vmaps[vid] && vmaps[vid].b) {
      ...
      for (var i = 0; i < constraints.length; i++) {
        ...
        var pred = cons.p;
        if (!b.metadata[pred]) b.metadata[pred] = [];
        if (
          b.metadata[pred].push &&
  ...
  // Create binding data
  var bindings = [];
  for (var vid in vmaps) bindings.push(vmaps[vid]);
...
RunBrowser.prototype.getProvenance = function (runid, tabPanel) {
  var msgTarget = tabPanel.getEl();
  msgTarget.mask("Publishing RDF ...", "x-mask-loading");
  var url = this.op_url + "/provenance";
  Ext.Ajax.request({
    url: url,
    params: {
      run_id: runid,
    ...
    success: function (response) {
      ...
      if (resp.url) {
        ...
        var win = new Ext.Window({
          ...
          maximizable: true,
          html:
            "<div style='padding:5px'>URL: <a target='_blank' href='" +
            url +
            "'>" +
            url +
            "</a></div>" +
...
RunBrowser.prototype.prePublishList = function () {
  ...
  Ext.Msg.prompt(
    ...
    function (btn, text) {
      if (btn == "ok" && text) This.publishList(text);
      else {
          ...
          Ext.MessageBox.confirm(
            ...
            function (b) {
              if (b == "yes") {
                ...
                This.getProvenance(runid, panel);
                tab.getLoader().load();
              }
            }
          );
...
RunBrowser.prototype.setGraphRuntimeInfo = function (graph, data) {
  if (!graph || !graph.editor || !graph.editor.template) return;
...
RunBrowser.prototype.getRunLog = function (exec) {
  ...
  exec.queue.steps.sort(function (a, b) {
    ...
    } else if (a.runtimeInfo.endTime == null) return 1;
    else if (b.runtimeInfo.endTime == null) return -1;
    else
      return a.runtimeInfo.endTime > b.runtimeInfo.endTime
        ? 1
        : a.runtimeInfo.endTime < b.runtimeInfo.endTime
        ? -1
        : 0;
  });
  for (var i = 0; i < exec.queue.steps.length; i++) {
    ...
    if (
      step.runtimeInfo.status != "WAITING" &&
...
RunBrowser.prototype.refreshOpenRunTabs = function (grid, wRunStore) {
  ...
  if (selectedTab && selectedTab.runid) this.selectRunInList(selectedTab.runid);
};
  ...
  grid.getSelectionModel().on(
    ...
    function (item, oldrec, rec, opts) {
      if (rec) {
        ...
        else grid.down("#stopButton").setDisabled(true);
...
RunBrowser.prototype.selectRunInList = function (runid) {
  var This = this;
  if (!runid) this.runList.getSelectionModel().deselectAll();
...
RunBrowser.prototype.initialize = function () {
  ...
  if (!This.runid) This.tabPanel.setActiveTab(0);
  return mainpanel;
};
